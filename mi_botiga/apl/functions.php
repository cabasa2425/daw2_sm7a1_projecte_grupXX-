...
function showManagers(){
            ...
            foreach ($lines as $line){
                ...
                if (isset($parts[2]) && $parts[2] === "manager") {
    ...
    function showClients(){
        ...
        foreach ($lines as $line){
            ...
            if (isset($parts[2]) && $parts[2] === "client") {
...
function clientDeleteManage($user_id, $action, $modification_details) {
    $user_id = $_POST['id'];
    $action = $_POST['action'];
    $modification_details = $_POST['modification_details'];
    ...
    try {
        ...
        $mail->Subject = "Reporte de Cliente - Acción: " . ucfirst($action);  // Asunto del correo
        $mail->isHTML(true);
        $mail->Body = "<h1>Nuevo Reporte de Acción</h1>
                       <p><strong>ID del Usuario:</strong> {$user_id}</p>
                       <p><strong>Acción:</strong> {$action}</p>
                       <p><strong>Detalles de la Modificación:</strong><br>{$modification_details}</p>";

        // Enviar correo
        if ($mail->send()) {
            echo 'El correo se ha enviado correctamente.';
            header('location: index.php'); // Redirigir después de enviar el correo
        } else {
            echo 'No se pudo enviar el correo. Inténtalo nuevamente.';
        }
    } catch (Exception $e) {
