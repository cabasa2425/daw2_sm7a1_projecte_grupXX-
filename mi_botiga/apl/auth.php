...
if (($_SERVER['REQUEST_METHOD'] === 'POST') && isset($_GET['filter']) && ($_GET['filter'] !== 'modifyClient')) {
    $name = $_POST['name'] ?? "";
    $pwd = $_POST['pwd'];
    $id = 0;
    $email = $_POST['email'] ?? "";
...
if (isset($pdf) && $pdf === "client") {
    ClientToPDF();
} elseif (isset($pdf) && $pdf === "manager") {
    ManagerToPDF();
} 
?>
...
<header>
    ...
    <?php if ($filter === 'login'): ?>
        ...
        <?php if( isset($mensaje) && ($mensaje)): echo "<h1>$error</h1>"?>  <?php endif; ?>
